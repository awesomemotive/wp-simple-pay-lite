...
abstract class Util
{
    ...
    /**
     * Converts a response from the SimplePay\Vendor\Stripe API to the corresponding PHP object.
     *
     * @param array                $resp    the response from the SimplePay\Vendor\Stripe API
    ...
    {
        $types = 'v1' === $apiMode ? \SimplePay\Vendor\Stripe\Util\ObjectTypes::mapping
            : \SimplePay\Vendor\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object'])
                ...
                if ('v2' === $apiMode && ('v2.core.event' === $resp['object'])) {
                    $eventTypes = \SimplePay\Vendor\Stripe\Util\EventTypes::thinEventMapping;
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        $class = $eventTypes[$resp['type']];
                    } else {
                        $class = \SimplePay\Vendor\Stripe\StripeObject::class;
                    }
                }
                ...
                $class = \SimplePay\Vendor\Stripe\V2\Collection::class;
            } else {
                $class = \SimplePay\Vendor\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \SimplePay\Vendor\Stripe\RelatedObject();
                    ...
                    $reason = new \SimplePay\Vendor\Stripe\Reason();
    ...
    {
        if ($h instanceof \SimplePay\Vendor\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
