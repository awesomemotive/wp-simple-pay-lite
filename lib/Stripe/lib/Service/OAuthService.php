...
class OAuthService extends \SimplePay\Vendor\Stripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \SimplePay\Vendor\Stripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \SimplePay\Vendor\Stripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \SimplePay\Vendor\Stripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \SimplePay\Vendor\Stripe\StripeClient([clientId => <CLIENT-ID>
                ])`)".  You can find your client_ids '
              . 'in your SimplePay\Vendor\Stripe dashboard at '
     ...
     *
     * @return \SimplePay\Vendor\Stripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \SimplePay\Vendor\Stripe\Service\OAuthService from \SimplePay\Vendor\Stripe\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \SimplePay\Vendor\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \SimplePay\Vendor\Stripe\Util\RequestOptions::parse($opts);
    }
