...
function CardToday() {
	const report = useRestApiReport(
		'/wpsp/__internal__/report/today',
		{
			currency,
		},
		[]
					...
					<>
						...
						{ topForms.length > 0 && (
							<>
								...
								<h3
									className={ `${ baseClassName }-today-misc-title` }
								>
									{ __( 'Top Forms', 'simple-pay' ) }
								</h3>

								<div className={ `${ baseClassName }-forms` }>
									{ topForms.map( ( form ) => {
										return (
											<TopForm
												key={ form.id }
						...
						{ tip && (
							<div className={ `${ baseClassName }-tip` }>
								<strong>
									<svg
