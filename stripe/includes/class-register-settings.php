...
class Stripe_Checkout_Register_Settings {
		...
		foreach( $sc_settings as $setting => $option ) {
			...
			foreach ( $sc_settings[$setting] as $option ) {
				add_settings_field(
					...
					$option['name'],
					method_exists( 'Stripe_Checkout_Register_Settings', $option['type'] . '_callback' ) ? array( 'Stripe_Checkout_Register_Settings', $option['type'] . '_callback' ) : array( 'Stripe_Checkout_Register_Settings', 'missing_callback' ),
					'sc_settings_' . $setting,
					'sc_settings_' . $setting,
					self::get_settings_field_args( $option, $setting )
				);
			}

			register_setting( 'sc_settings_' . $setting, 'sc_settings_' . $setting, array( 'Stripe_Checkout_Register_Settings', 'settings_sanitize' ) );
	...
	public static function toggle_control_callback( $args ) {
		global $sc_options;
	...
	public static function radio_callback( $args ) {
		...
		foreach ( $args['options'] as $key => $option ) {
			...
			if ( isset( $sc_options[ $args['id'] ] ) && $sc_options[ $args['id'] ] == $key ) {
				$checked = true;
			} else if( isset( $args['std'] ) && $args['std'] == $key && ! isset( $sc_options[ $args['id'] ] ) ) {
				$checked = true;
			}
	...
	public static function license_callback( $args ) {
		global $sc_options;
	...
	public static function get_settings_field_args( $option, $section ) {
		$settings_args = array(
			'id'      => $option['id'],
			'desc'    => $option['desc'],
			'name'    => $option['name'],
