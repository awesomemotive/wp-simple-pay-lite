...
if ( ! class_exists( 'Stripe_Checkout_System_Status' ) ) {
    class Stripe_Checkout_System_Status
    {
        ...
        public function set_admin_tab_content()
        {
            ...
            if ($memory < 67108864) {
                $memory = '<mark class="error">' . sprintf(__('%1$s - It is reccomendend to set memory to at least 64MB. See: <a href="%2$s" target="_blank">Increasing memory allocated to PHP</a>', 'sc'), $memory_export, 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP') . '</mark>';
            } else {
                    ...
                    <table class="widefat sc-system-status-report-panel">
                        ...
                        <?php foreach ($sections[$panel] as $row => $cell) : ?>
                            <tr>
                                ...
                                <td class="label"
                                    data-export="<?php echo trim($label_export); ?>"><?php echo $cell['label']; ?></td>
                                <td class="result"
                                    data-export="<?php echo trim($result_export); ?>"><?php echo $cell['result']; ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
